...
script_name = "comparative_tasic_correlation_glut_by_injection_group_regress"
...
fname_avg_ip2_list = file.path(tas_data_dir, sprintf("%s_seurat_glut_data_avg_injection_target_category2_list.rds", fp_name))
...
tas_samp_n = tas_samp %>% group_by(injection_primary) %>%
  summarize(n=n()) %>%
  filter(n>=30)
tas_samp1 = tas_samp %>% 
  inner_join(tas_samp_n) %>% 
  distinct(injection_primary, injection_category) %>% 
  mutate(injection_category2 = case_when(grepl("BLA", injection_primary)~"BLA",
    grepl("CTX", injection_category)~ "NEO",
                                         injection_primary == "GRN" ~ "MY",
                                         injection_primary == "PARN" ~ "MY",
                                         injection_primary %in% c("SSp", "ORBl-c", "AId", "MOs") ~ "NEO",
                                         injection_primary %in% c("CP") ~ "STR", 
                                         injection_primary == "VAL" ~ "TH",
                                         grepl("VIS", injection_primary) ~ "NEO",
                                         grepl("AUD", injection_primary) ~ "NEO",
                                         grepl("MOs|SSs|ALM", injection_primary) ~ "NEO",
                                         injection_primary=="CLA" ~ "CLA",
                                        injection_primary=="DG" ~ "HPC",
                                        injection_primary=="LGd" ~ "NONE",
                                        is.na(injection_category)~"NONE",
                                        TRUE~injection_category)) %>%
  mutate(injection_category2 = factor(injection_category2, 
                                     levels=c("NEO", "BLA", "HPC", "CLA", "STR", "TH", "TEC", "MY", "P", "NONE"),
                                     labels = c("Neocortex", "BLA", "HPC", "Claustrum", "Striatum", "Thalamus", "Tectum", "Medulla", 
                                                "Pons", "No injection")))
md = FetchData(tas_obj, "injection_primary")
md = md %>% rownames_to_column() %>%
  left_join(tas_samp1) %>%
  column_to_rownames()

tas_obj = AddMetaData(tas_obj, md)
redo_avg = T
if (redo_avg) {

  
  Idents(tas_obj) = tas_obj@meta.data$injection_category2
  tas_obj_cur = subset(tas_obj, subset=injection_category2!="No injection")
  table(tas_obj_cur$injection_category2)
  tas_obj_avg_ip2 = AverageExpression(tas_obj_cur, assays = c("SCT"), slot="data")
  
  
  tas_obj_avg_ip2 = map(tas_obj_avg_ip2, function(x) {
    rownames(x) = tas_genes$gene_symbol[match(rownames(x), tas_genes$gene_entrez_id)]
    rownames(x) = toupper(rownames(x))
    x
  })
  
  saveRDS(tas_obj_avg_ip, fname_avg_ip2_list)
  
} else {
  tas_obj_avg_ip2 = readRDS(fname_avg_ip2_list)
}

tas_obj_avg_ip_sct = tas_obj_avg_ip2[["SCT"]]
...
tas_ip2_markers_fname = file.path(tas_data_dir, "tas_obj_injection_category2_markers.rds")
redo_tas_markers = T
if (redo_tas_markers) {
  cells = Cells(tas_obj)[!(tas_obj$injection_category2 %in% c("No injection"))]
  tas_obj_cur = subset(tas_obj, cells=cells)
  Idents(tas_obj_cur) = tas_obj_cur$injection_category2
  ...
  genes_to_test = genes_to_test[genes_to_test %in% rownames(tas_obj_cur)]
  tas_ip_markers = FindAllMarkers(tas_obj_cur, 
                                   assay="SCT", 
                                   features = genes_to_test, 
                                   test.use = "t",
                                   min.pct = .2, only.pos=T,max.cells.per.ident = 200
  )
  tas_ip_markers = tas_ip_markers %>%
    rename(entrez_id = gene) %>%
    mutate(gene = tas_genes$gene_symbol[match(entrez_id, tas_genes$gene_entrez_id)]) 
  tas_ip_markers = tas_ip_markers %>%
    mutate(gene = toupper(gene))
  saveRDS(tas_ip_markers, tas_ip_markers_fname)
} else {
  tas_ip_markers = readRDS(tas_ip_markers_fname)
}
...
mm_mat_filt = tas_obj_avg_ip_sct[rownames(tas_obj_avg_ip_sct) %in% rownames(obj_int_filt_avg),]
...
ngenes = 200
p_adj_filt = .05
tas_ip_markers_sig = tas_ip_markers %>% 
  group_by(cluster) %>%
  filter(p_val_adj < p_adj_filt) %>% 
...
obj_int_filt_markers_to_use = obj_int_filt_markers %>%
  ...
  filter(p_val_adj < p_adj_filt) %>% 
  top_n(ngenes, avg_logFC) %>%
  filter(gene %in% tas_ip_markers_sig$gene)

obj_int_filt_avg_filt = obj_int_filt_avg_filt[rownames(obj_int_filt_avg_filt) %in% obj_int_filt_markers_to_use$gene,]

mm_md_n = tas_samp
mm_mat_filt = mm_mat_filt[,!grepl("Not Reviewed", colnames(mm_mat_filt))]
mm_mat_filt = mm_mat_filt[,!(colnames(mm_mat_filt) %in% c("unknown"))]
...
pheatmap(obj_cor1, 
         ...
         filename=file.path(out_dir, "injection_markers_specificity_heatmap_vert.pdf"), height=5, width=5.5)
...
pheatmap(obj_cor1, 
         ...
         breaks=breaks,
         display_numbers = t(obj_cor_sig_adj_label),
         filename=file.path(out_dir, "injection_markers_specificity_heatmap_vert_no_names.pdf"), height=5, width=3.5)
 ...
 pheatmap(t(obj_cor1), 
         ...
         filename=file.path(out_dir, "mouse_injection_bf_neurons_exc_markers_specificity_heatmap_horiz.pdf"), height=3, width=8)


 ct_order = levels(tas_samp1$injection_category2)
 ct_order2 = c("HVC_Glut-1", "HVC_Glut-4", "HVC_Glut-2", "HVC_Glut-5",  "HVC_Glut-3", "RA_Glut-1", "RA_Glut-2", "RA_Glut-3")
 obj_cor2 = obj_cor1[intersect(ct_order, rownames(obj_cor1)),ct_order2]
 obj_cor_sig_adj_label_order = obj_cor_sig_adj_label[ct_order2,intersect(rownames(obj_cor2), colnames(obj_cor_sig_adj_label))]
 
 obj_cor2_floor = obj_cor2
 obj_cor2_floor[obj_cor2_floor<0] = 0 
 
 colors =  colorRampPalette(brewer_pal(palette="Blues")(9))(palette_length)
 breaks =  seq(0, max(obj_cor2_floor), length.out=palette_length+1)
 
 pheatmap(obj_cor2_floor, 
          annotation_names_row = F,
          annotation_names_col = F,
          show_colnames=T, 
          cluster_cols= F, 
