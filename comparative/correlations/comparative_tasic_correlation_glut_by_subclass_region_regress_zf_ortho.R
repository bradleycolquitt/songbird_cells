...
script_name = "comparative_tasic_correlation_glut_by_subclass_region_regress_zf_ortho"
...
fname_avg_region_sc_list = file.path(tas_data_dir, sprintf("%s_seurat_glut_data_avg_region_subclass_list.rds", fp_name))
...
tas_obj$brain_region_subclass = paste(tas_obj$brain_region, tas_obj$subclass, sep="-")
tab = FetchData(tas_obj, c("brain_region", "subclass")) %>% table() %>%
  as.data.frame() %>% 
  filter(Freq>50) %>%
  mutate(region_subclass = paste(brain_region, subclass, sep="-"))

cells = Cells(tas_obj)[tas_obj$brain_region_subclass %in% tab$region_subclass]
tas_obj = subset(tas_obj, cells=cells)

redo_avg = F
if (redo_avg) {
  Idents(tas_obj) = tas_obj$brain_region_subclass
  tas_obj_avg1 = AverageExpression(tas_obj, assays = c("SCT"), slot="data")
  saveRDS(tas_obj_avg1, fname_avg_region_sc_list)
} else {
  tas_obj_avg1 = readRDS(fname_avg_region_sc_list)
}

tas_obj_avg_sc_sct = tas_obj_avg1[["SCT"]]
...
tas_exc_markers_fname = file.path(tas_data_dir, "tas_obj_glut_ortho_zf_brain_region_subclass_markers.rds")
redo_tas_markers = F
if (redo_tas_markers) {
...
ct_order = c("ALM-L2/3 IT", "VISp-L2/3 IT", "VISp-L4",
             "ALM-L5 IT", "VISp-L5 IT", "ALM-L5 PT", "VISp-L5 PT",
             "ALM-L6 IT", "VISp-L6 IT",  
             "ALM-L6 CT", "VISp-L6 CT",
             "ALM-L6b", "VISp-L6b")
