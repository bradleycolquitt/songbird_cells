...
script_name = "comparative_tasic_correlation_glut_by_injection_regress_zf_ortho"
...
fname_avg_ip2_list = file.path(tas_data_dir, sprintf("%s_seurat_glut_data_avg_injection_target_list.rds", fp_name))
...
tas_obj_avg_ip_sct = tas_obj_avg_ip[["SCT"]]
...
tas_exc_markers_fname = file.path(tas_data_dir, "tas_obj_glut_ortho_zf_injection_primary_markers.rds")
redo_tas_markers = F
if (redo_tas_markers) {
  
  Idents(tas_obj) = tas_obj$injection_primary
  genes_to_test = intersect(rownames(tas_obj), rownames(obj_int_filt))
  cells = Cells(tas_obj)[!(tas_obj$injection_primary %in% c("No Injection", "unknown"))]
...
mm_mat_filt = mm_mat_filt[, colnames(mm_mat_filt)!="unknown"]
mm_mat_filt = mm_mat_filt[,colnames(mm_mat_filt) %in% tas_samp1$injection_primary]
...
ann_row = tas_samp1 %>% distinct(injection_primary, .keep_all=T) %>% 
 select(injection_primary, injection_category2) %>%
  filter(injection_primary %in% rownames(mat_cur)) %>% 
  column_to_rownames("injection_primary")

ind = match(rownames(mat_cur), rownames(ann_row))
na = rownames(ann_row)
ann_row = data.frame(injection_category2 = ann_row$injection_category2[ind])
rownames(ann_row) = na[ind]


regions = na.omit(unique(ann_row$injection_category2))
region_colors = qualpalr::qualpal(length(regions))$hex
names(region_colors) = regions


ha = rowAnnotation(df = ann_row, 
                   annotation_name_gp =  gpar(fontsize=6, fontface="bold"),
                   col = list(injection_category2 = region_colors),
                   annotation_width=unit(target_width*.2, "in"),
                   simple_anno_size_adjust=T,
                   annotation_legend_param = list(
                     injection_category2 = list(gp = gpar(fontsize=6))
                    ))

hm = Heatmap(mat_cur, col=colors, name = "rho",
