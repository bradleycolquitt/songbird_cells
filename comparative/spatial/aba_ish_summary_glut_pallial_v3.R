...
script_name = "aba_ish_summary_glut_pallial_v3"
...
#obj_int_avg1_trans = log1p(obj_int_avg2)
#obj_int_avg1_trans = t(apply(obj_int_avg1_trans, 1, function(x) (x - mean(x)) / sd(x)))
...
mat_a_scale = t(apply(mat_a, 1, function(x) scale(x)))
mat_b_scale = t(apply(mat_b, 1, function(x) scale(x)))
colnames(mat_a_scale) = colnames(mat_a)
colnames(mat_b_scale) = colnames(mat_b)
pdf(file.path(out_dir, "mat_a.pdf"), height=15, width=6)
Heatmap(mat_a, cluster_columns=F)
dev.off()

pdf(file.path(out_dir, "mat_a_spec.pdf"), height=15, width=6)
Heatmap(mat_a_spec, cluster_columns=F)
dev.off()

pdf(file.path(out_dir, "mat_a_scale.pdf"), height=15, width=6)
Heatmap(mat_a_scale,cluster_columns=F)
dev.off()

pdf(file.path(out_dir, "mat_b.pdf"), height=15, width=20)
Heatmap(mat_b, cluster_columns=F)
dev.off()

pdf(file.path(out_dir, "mat_b_spec.pdf"), height=15, width=20)
Heatmap(mat_b_spec, cluster_columns=F)
dev.off()

pdf(file.path(out_dir, "mat_b_scale.pdf"), height=15, width=20)
Heatmap(mat_b_scale,cluster_columns=F)
dev.off()

plot(mat_a_spec[,"RA_Glut-1"], mat_b_spec[,"PERI5"])

spec_df = data.frame(cbind(mat_a_spec, mat_b_spec))
spec_df_rank = data.frame(apply(spec_df, 2, rank))
gg = ggplot(spec_df_rank, aes(HVC_Glut.1, AONpv)) + 
  geom_point() + 
  stat_smooth(method="lm")
gg

scale_df = data.frame(cbind(mat_a_scale, mat_b_scale))
scale_df_rank = data.frame(apply(scale_df, 2, rank))
gg = ggplot(scale_df_rank, aes(HVC_Glut.1, AONpv)) + 
  geom_point() + 
  stat_smooth(method="lm")
gg

# #mat_a = mat_a[match(rownames(mat_b), rownames(mat_a)),]
# mat_a_spec = t(apply(mat_a, 1, function(x) log(x / mean(x))))
# mat_b_spec = na.omit(t(apply(mat_b, 1, function(x) log(x / mean(x)))))
# mat_a_spec = mat_a_spec[match(rownames(mat_b_spec), rownames(mat_a_spec)),]
# 
# obj_cor = cor(mat_a_spec, mat_b_spec, method="spearman")
# 
# #mat_a = mat_a_spec
# #mat_b = mat_b_spec
# obj_cor_sig = matrix(ncol = ncol(obj_cor), nrow=nrow(obj_cor), dimnames = list(colnames(mat_a), colnames(mat_b)))
# 
# for (i in 1:ncol(mat_a_spec)) {
#   for (j in 1:ncol(mat_b_spec)) {
#     obj_cor_sig[i,j] = cor.test(mat_a_spec[,i], mat_b_spec[,j], method="spearman", exact=F)$p.value
#   }
# }
# 
...
# obj_cor_sig_adj = t(apply(obj_cor_sig, 1, p.adjust, method="BH"))
# sig_thresh = .1
...
obj_cor_seg = obj_cor_df %>%
  ...
  top_n(20, value) %>% 
  ungroup() %>% 
  left_join(taxon_md_df1 %>% select(acronym, prefix1)) %>%
  group_by(prefix1, celltype ) %>%
  summarize(value = n()) %>%
  left_join(taxon_md_df2 %>% select(prefix1, xend1, yend, taxon_width)) %>%
  left_join(finch_samp) %>%
  #distinct(prefix1, celltype, .keep_all=T) %>% 
  mutate(value_scale = value * size_scale) #%>%
#mutate(position2 = str_extract(celltype, "^[A-Z]+")) %>%
...
gg = ggplot() + 
  geom_segment(data=obj_cor_seg, aes(x=x, y=y, xend = xend1, yend = yend+(finch_height/2) + (finch_height/2),
                                     alpha=value,
                                     color=position2_color),
               size=.25) +
  geom_tile(data=taxon_md_df2_un, aes(xend1, yend-(finch_height/2), fill=cortical_color, 
                                      width=taxon_width,
                                      height=finch_height
                                      ), color=1) + 
  #geom_label(data=taxon_md_df2_un_cortical, aes(xend2, yend+(finch_height/2), label=cortical, ), fill="white", size=7/2.83, label.size=0) + 
  geom_tile(data=taxon_md_df2_un, aes(xend1, yend+(finch_height/2), fill=pallial_color, 
                                      width=taxon_width,
                                      height=finch_height), color=1) + 
  #geom_label(data=taxon_md_df2_un_pallial, aes(xend2, yend-(finch_height/2), label=pallial), fill="white", size=7/2.83, label.size=0) + 
  geom_tile(data=finch_samp, aes(x,y, width=finch_width, height=finch_height, fill=position2_color), color=1) + 
  #geom_text(data=finch_samp, aes(x=x, y = y + finch_height, label=celltype), size=7/2.83) + 
  geom_text(data=finch_samp, aes(x=x, y = y, label=celltype_num), size=7/2.83) + 
...
gg
save_plot(file.path(out_dir, "finch_aba_cor_region_alpha.pdf"), gg, base_width=xmax /12, base_height = ymax/12)

gg_legend = get_legend(gg)
save_plot(file.path(out_dir, "finch_aba_cor_region_alpha_legend.pdf"), gg_legend, base_width=5, base_height=5)
...
taxon_md_df2_un = taxon_md_df2 %>% 
  distinct(prefix1, yend, xend1, cortical, pallial) %>%
  mutate(cortical_color = cortical_colors[cortical],
         pallial_color = pallial_colors[pallial])
...
obj_cor_seg = obj_cor_df %>%
  ...
  #top_n(20, value) %>% 
  ungroup() %>% 
  left_join(taxon_md_df1 %>% select(acronym, pallial)) %>%
  group_by(pallial, celltype ) %>%
  summarize(value = n()) %>%
  left_join(taxon_md_df2 %>% select(pallial, xend, yend)) %>%
  left_join(finch_samp) %>%
  #distinct(prefix1, celltype, .keep_all=T) %>% 
  mutate(value_scale = value * size_scale) #%>%
#mutate(position2 = str_extract(celltype, "^[A-Z]+")) %>%
...
gg
save_plot(file.path(out_dir, "finch_aba_cor_region_alpha.pdf"), gg, base_height=xmax /12, base_width = ymax/6)
...
obj_cor_df_filt_sum3 = region_calc_summary(obj_cor_df_cur,  taxon_md_df=taxon_md_df2, cor_quantile_filter = .95, value_n_quantile_filter = .95, grouping_factor_compare = "cortical",
                                           grouping_factor_data = "position2")
...
gg_cor_bar = ggplot(obj_cor_df_filt_sum3, aes(cortical, value_n_n_fc_shuf, color=position2, fill=position2)) + 
  geom_hline(yintercept=1, linetype=2, size=1/2.83) + 
  geom_bar(stat="identity", position=position_dodge(width = .75), width=.5)  + 
  # geom_linerange(aes(ymin = 0, ymax = value_n_n_fc_shuf, x=pallial), position=position_dodge(width=.5)) + 
  coord_flip() +
  ...
  ylim(0, 5)
gg_cor_bar
save_plot(file.path(out_dir, "obs_exp_sig_cor_cortical_bar.pdf"), gg_cor_bar, base_height=1.5, base_width=2)

obj_cor_df_filt_sum3 = obj_cor_df_filt_sum3 %>%
  mutate(cortical = factor(cortical, levels=rev(levels(cortical))))

gg_cor_bar_v = ggplot(obj_cor_df_filt_sum3, aes(cortical, value_n_n_fc_shuf, color=position2, fill=position2)) + 
  geom_hline(yintercept=1, linetype=2, size=1/2.83) + 
  geom_bar(stat="identity", position=position_dodge(width = .75), width=.5)  + 
  # geom_linerange(aes(ymin = 0, ymax = value_n_n_fc_shuf, x=pallial), position=position_dodge(width=.5)) + 
  #coord_flip() +
...
save_plot(file.path(out_dir, "obs_exp_sig_cor_cortical_bar_vert.pdf"), gg_cor_bar_v, base_height=1.5, base_width=1.5)
...
pg = plot_grid(gg_pal_bar, gg_cor_bar, nrow=2, align="v", axis="lr")
pg
save_plot(file.path(out_dir, "obs_exp_sig_cor_pallial_cortical_bar.pdf"), pg, base_height=1.5, base_width=2, nrow=2)

pg = plot_grid(gg_pal_bar_v, gg_cor_bar_v, ncol=2, align="h", axis="tb")
pg
save_plot(file.path(out_dir, "obs_exp_sig_cor_pallial_cortical_bar_vert.pdf"), pg, base_height=1.5, base_width=2, ncol=2)
...
#obj_cor_df = melt(obj_cor)
#colnames(obj_cor_df) = c("celltype", "acronym", "cor")
...
obj_cor_df_top %>% rowwise() %>% do({
  ...
  print(acronym_cur)
  med_pos = ref_dt_med %>% filter(grepl(acronym_cur, acronym))
  slice_num = case_when(plane=="saggital" ~ med_pos$Y[1])
  ish_plot = ish_slice_heatmap_flat(mat=grid_anno_cors[[ct]],
                                    anno = reference,
                                    taxon = taxon_filt,
                                    slice_num = slice_num,
...
# aba_svgs = map(aba_ids, ~get_aba_svg(.x, 4))
# 
# 
